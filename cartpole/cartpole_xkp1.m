...
%
    ...
    sgn_DxtNc = sign(Dxt);
    D2thetat = calculate_D2thetat2(Dthetat, Dthetat_sq, F, g, l, mp, ...
        muc, mup, sin_theta, cos_theta, invl, inv_mcmp, sgn_DxtNc);

    % Now compute Nc using this D2thetat value
    Nc = (-g .* mcmp + l .* mp ...
        .* (D2thetat .* sin_theta + cos_theta .* Dthetat_sq) ...
    ) .* 1.0e-3;
    ...
    if sign(Nc) == -1
        ...
        sgn_DxtNc = -sgn_DxtNc;
        D2thetat = calculate_D2thetat2(Dthetat, Dthetat_sq, F, g, l, ...
            mp, muc, mup, sin_theta, cos_theta, invl, inv_mcmp, sgn_DxtNc);
    end

    D2xt = inv_mcmp .* ( ...
            F + l .* mp .* (sin_theta .* Dthetat_sq - D2thetat .* cos_theta) ...

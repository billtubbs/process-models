function eqs = craneDAEFunction(t,Y,YP,params)
...
% 
    ...
    eqs = [
         - Dxt + v_x - Dthetat .* L .* cos_theta;
        v_y - Dthetat .* L .* sin_theta;
        F_d - (c_d .* rho .* r_sq .* pi .* (v_x_sq + v_y_sq)) ./ 2.0;
        N_x - m_p .* (YP10 + YP11 .* L .* cos_theta - L .* sin_theta .* Dthetat_sq);
        N_y - m_p .* (g - L .* (YP11 .* sin_theta + cos_theta .* Dthetat_sq));
        N_c - N_y - g .* m_c;
        F_f - N_c .* muc .* sign(Dxt .* N_c);
        YP10 + inv_mcmp .* (F_f - F + L .* m_p .* (YP11 .* cos_theta - sin_theta .* Dthetat_sq));
         - YP11 + ((cos_theta .* (m_p .* inv_mcmp .* ( - F_f + F + L .* m_p .* sin_theta .* Dthetat_sq) + (c_d .* rho .* r_sq .* v_x_sq .* pi .* sign(v_x)) ./ 2.0) - sin_theta .* (g .* m_p - (c_d .* rho .* r_sq .* v_y_sq .* pi .* sign(v_y)) ./ 2.0)) .* (3.0 ./ 4.0)) ./ (L .* m_p .* (m_p .* cos_theta.^2 .* inv_mcmp .* (3.0 ./ 4.0) - 1.0));
        Dxt - YP1;
        Dthetat - YP2
    ];

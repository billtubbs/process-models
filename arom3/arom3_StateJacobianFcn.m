...
%       6e5 / 1e5];  % (normalized)
    ...
    dfdx(1, 1) = 1 - dt*( ...
            (xk(2)*rate_const*(2*cD(3) + 6*cD(2)*xk(1) + 12*cD(1)*xk(1)^2)) ...
            / (Cp*rho) + (Ea^2*xk(2)*rate_const*DeltaH) / (Cp*R^2*rho*xk(1)^4) ...
            + (2*Ea*xk(2)*rate_const*dDeltaHdx1)/(Cp*R*rho*xk(1)^2) ...
            - (2*Ea*xk(2)*rate_const*DeltaH)/(Cp*R*rho*xk(1)^3) ...
        );

    dfdx(1, 2) = -dt*( ...
            (rate_const*dDeltaHdx1)/(Cp*rho) ...
            + (Ea*rate_const*DeltaH)/(Cp*R*rho*xk(1)^2) ...
        );
